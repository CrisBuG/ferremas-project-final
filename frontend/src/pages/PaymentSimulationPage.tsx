...
import { apiClient, transbankService } from '../services/api';
...
const IconContainer = styled.div`
  font-size: 4rem; // Reducido de 3.5rem a 2.5rem para un tamaño más apropiado
  ...
  svg {
    ...
    width: 2.5rem; // Tamaño fijo para el ícono
    height: 2.5rem; // Tamaño fijo para el ícono
  }
`;
...
const PaymentSimulationPage: React.FC = () => {
    ...
    const handlePaymentResult = async (result: 'approved' | 'rejected') => {
        setLoading(true);
        try {
          // Obtener el token de la URL (viene de Transbank)
          ...
          console.log('Confirmando transacción simulada con token:', token_ws);
          
          // Usar la nueva API de confirmación simulada
          const response = await transbankService.confirmSimulationTransaction(token_ws);

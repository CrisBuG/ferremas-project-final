...
// Interceptor para agregar JWT token y manejar FormData
...
apiClient.interceptors.response.use(
  ...
  if (error.response?.status === 401) {
    // Evitar bucles de redirección y respetar HashRouter
    // Detecta ruta actual usando hash si existe
    ...
    const isPaymentSimulationPage = 
      ...
      currentRoute.includes('/payment-simulation');

    if (!isOnLogin && !isPaymentSimulationPage) {
      // Si se usa HashRouter, redirigir con '#/login'

...
def login_view(request):
    print("[login_view] POST received")
    ...
    if email and password:
        ...
        if user:
            print(f"[login_view] Auth OK for {email}")
            login(request, user)
            ...
            return Response({
                ...
                'user': {
                    'id': user.id,
                    'username': user.email,  # Alias de username para compatibilidad
                    'email': user.email,
            ...
            print(f"[login_view] Invalid credentials for {email}")
            return Response({
                'success': False,
...
def register_view(request):
    ...
    # username puede venir desde el cliente pero el modelo usa email como identificador
    ...
    # No validar username en el modelo personalizado
    ...
    try:
        # CustomUser usa email como identificador; no incluye username
        ...
        # Autenticar para adjuntar backend antes de login
        user_auth = authenticate(username=email, password=password) or authenticate(username=username, password=password)
        if user_auth:
